...
// Données mock avec structure réelle Mosyle
// TODO: Remplacer par un appel API réel : GET /v1/listdevices
...
function EraseConfirmDialog({ device, onConfirm, onCancel }: EraseConfirmDialogProps) {
  ...
  const isReady = confirmText === 'CONFIRMER';
...
export default function MDMPanel() {
  ...
  const handleLock = async (device: Device) => {
    ...
    try {
      // TODO: Appel API réel Mosyle
      // PUT /v1/mdm/{deviceId}/lock
      // Authorization: Bearer MOSYLE_API_KEY
      //
      // const res = await fetch(`/api/mdm/lock`, {
      //   method: 'POST',
      //   headers: { 'Content-Type': 'application/json' },
      //   body: JSON.stringify({ serialNumber: device.serialNumber }),
      // });

      // Simulation (supprimer en production)
  ...
  const handleEraseConfirm = async () => {
    ...
    try {
      // TODO: Appel API réel Mosyle
      // PUT /v1/mdm/{deviceId}/erase
      // Authorization: Bearer MOSYLE_API_KEY
      // Nécessite rôle super_admin + MFA validé
      //
      // const res = await fetch(`/api/mdm/erase`, {
      //   method: 'POST',
      //   headers: { 'Content-Type': 'application/json' },
      //   body: JSON.stringify({ serialNumber: eraseTarget.serialNumber }),
      // });
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="ml-auto flex items-center gap-1.5 text-xs text-text-muted">
            ...
            <span>
              {onlineCount}/{MOCK_DEVICES.length} en ligne
            </span>
          </div>
        </div>
