...
export function SovereignLockout() {
  ...
  const isLocked = config.status?.maintenanceMode || config.status?.licenceStatus === 'expired' || config.status?.killSwitch;

  const getLockReason = () => {
    if (config.status?.killSwitch) return "SOUVERAINETÉ_RÉVOQUÉE";
    if (config.status?.maintenanceMode) return "STABILISATION_SYSTÈME";
    if (config.status?.licenceStatus === 'expired') return "LICENCE_EXPIREE";
