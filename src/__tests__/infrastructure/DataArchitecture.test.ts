...
describe('Data Architecture & Multi-Tenant Infrastructure', () => {
  it('should correctly scope paths to the active tenant context', () => {
    ...
    const resolved = TenantPathResolver.resolve('ops_flows', ctx);
    expect(resolved).toBe('tenants/tenant_lyon_01/ops_flows');
  });

  it('should prevent cross-tenant access when path explicitly targets another tenant', () => {
    const ctx = { vassalId: 'tenant_lyon_01', actorId: 'user_123' };
    const isValid = TenantPathResolver.validate('tenants/tenant_paris_02/ops_flows', ctx);
  ...
  it('should perform CRUD operations using SqliteMemoryAdapter in an agnostic manner', async () => {
    ...
    await adapter.set('tenants/tenant_test/ops_flows/ord_1', { id: 'ord_1', total: 4500, status: 'pending' }, undefined, ctx);
    const item = await adapter.get<{ id: string; total: number; status: string }>('tenants/tenant_test/ops_flows/ord_1', ctx);

    expect(item).not.toBeNull();
    expect(item?.total).toBe(4500);

    const queried = await adapter.query<{ id: string; total: number }>('tenants/tenant_test/ops_flows', {
