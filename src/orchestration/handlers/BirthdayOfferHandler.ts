    ...
    return NexusEventBus.on('crm.birthday_approaching', async (payload) => {
      ...
      try {
        ...
        if (customer?.email) {
          ...
          const merchantName = tenantSettings?.name || 'Notre restaurant';
          ...
          await NotificationGateway.send({
            tenantId,
            to: customer.email,
            subject: `${merchantName} — Joyeux anniversaire ${customer.firstName || ''} !`,
            text: [
              `Bonjour ${customerName},`,
              ``,
              `Toute l'équipe de ${merchantName} vous souhaite un très bon anniversaire !`,
              ``,
