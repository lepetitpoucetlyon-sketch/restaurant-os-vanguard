            ...
            <div key={ticket.id} className="bg-surface-card border border-subtle rounded-2xl p-5 space-y-3 shadow-sm hover:border-amber-500/30 transition-all">
              <div className="flex items-start justify-between gap-4">
                <div className="space-y-1">
                  <span className="text-[10px] font-mono text-text-tertiary"># {ticket.id}</span>
                  <h3 className="text-sm font-semibold text-text-primary">{ticket.description}</h3>
                </div>
                <div className="flex items-center gap-2 shrink-0">
                  {ticket.escalated && (
                    <span className="px-2.5 py-1 bg-rose-500/10 text-rose-500 border border-rose-500/20 rounded-full text-[10px] font-bold flex items-center gap-1">
