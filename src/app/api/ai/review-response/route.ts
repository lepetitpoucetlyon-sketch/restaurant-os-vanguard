 ...
 * Body: { reviewText: string; rating: number; merchantName: string }
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const { reviewText, rating, merchantName } = parsed.data;
    ...
    const prompt = `Génère une réponse professionnelle et chaleureuse en français à cet avis de restaurant : "${reviewText}". Le restaurant est ${merchantName}. Note : ${rating}/5. Sois spécifique au contenu de l'avis.`;
