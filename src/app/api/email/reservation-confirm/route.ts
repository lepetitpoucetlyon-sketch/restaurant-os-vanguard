...
function buildHtml({ name, date, time, covers, merchantName }: ConfirmPayload): string {
    const formattedDate = (() => {
        try {
...
<head>
  ...
  <title>Confirmation de réservation — ${merchantName}</title>
</head>
          ...
          <tr>
            <td style="padding:48px;">
              ...
              <p style="color:#999;font-size:14px;margin:0 0 40px;line-height:1.7;">
                ...
                <strong style="color:#c5a059;">${merchantName}</strong>.
          ...
          <tr>
            <td style="background:#111;padding:24px 48px;border-top:1px solid #2a2a2a;text-align:center;">
              <p style="color:#444;font-size:10px;letter-spacing:0.2em;text-transform:uppercase;margin:0;">
                &copy; ${new Date().getFullYear()} ${merchantName} — Restaurant OS
              </p>
            </td>
          </tr>
...
export async function POST(request: NextRequest): Promise<NextResponse> {
    ...
    try {
        ...
        const { to, name, date, time, covers, merchantName } = body;

        if (!to || !name || !date || !time || !covers || !merchantName) {
            return NextResponse.json({ error: 'Champs manquants' }, { status: 400 });
        }

        const payload: ConfirmPayload = { to, name, date, time, covers, merchantName };

        const { error } = await getResend().emails.send({
            from: FROM_EMAIL,
            to,
            subject: `Confirmation — ${merchantName} — ${date} à ${time}`,
