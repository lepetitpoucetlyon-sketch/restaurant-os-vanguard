...
export async function POST(req: Request): Promise<NextResponse> {
  try {
    ...
    logger.error('[EInvoicing/configure] POST', err);
...
export async function GET(req: Request): Promise<NextResponse> {
  try {
    ...
    if (tenantId) {
      ...
      const { tenantEInvoiceConfigPath } = await import('@/modules/finance/comptabilite/einvoicing');
    ...
    const { Nexus } = await import('@/lib/nexus/NexusAdapter');
    const { PLATFORM_EINVOICE_CONFIG_PATH } = await import('@/modules/finance/comptabilite/einvoicing');
    ...
    logger.error('[EInvoicing/configure] GET', err);
