...
export const POST = withTenantRoute(
  async (req: NextRequest, ctx): Promise<NextResponse> => {
    try {
      ...
      logger.error('[EInvoicing/outbound] POST', { error: err, correlationId: ctx.correlationId });
...
export const GET = withTenantRoute(
  async (req: NextRequest, ctx): Promise<NextResponse> => {
    try {
      ...
      logger.error('[EInvoicing/outbound] GET', { error: err, correlationId: ctx.correlationId });
