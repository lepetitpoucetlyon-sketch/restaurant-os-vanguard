...
export default function ProspectingDashboard() {
  ...
  const handleApplySauce = async (input: BrandInput) => {
    ...
    await updateConfig("theme", newTheme as unknown as import("@nexus/contracts").ThemeSettings);
    if (updateIdentity) await updateIdentity({ name: input.name, id: "identity_suture", updatedAt: new Date().toISOString() } as unknown as import("@nexus/contracts").RestaurantIdentity);
    setLastGenerated(newTheme as unknown as import("@nexus/contracts").ThemeSettings);
  };
