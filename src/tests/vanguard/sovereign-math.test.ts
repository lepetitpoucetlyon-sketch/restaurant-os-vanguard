...
describe('🏛️ SovereignMath : Microunits Protocol (Grade X)', () => {
    ...
    it('SC-1: Conversion vers Microunités (toMicrounits)', () => {
        ...
        expect(SovereignMath.toMicrounits(1.00)).toBe(BigInt(1_000_000));
        expect(SovereignMath.toMicrounits(0.1 + 0.2)).toBe(BigInt(300_000)); // Epsilon Guard
        expect(SovereignMath.toMicrounits(19.99)).toBe(BigInt(19_990_000));
    });

    it('SC-2: Conversion vers Euros (fromMicrounits)', () => {
        expect(SovereignMath.fromMicrounits(BigInt(1_000_000) as any)).toBe(1.00);
        expect(SovereignMath.fromMicrounits(BigInt(19_990_000) as any)).toBe(19.99);
    });
    ...
    it('SC-4: Arithmétique Souveraine (add/subtract/multiply)', () => {
        ...
        // Addition
        expect(SovereignMath.add(a, b)).toBe(BigInt(12_600_000));

        // Soustraction
        expect(SovereignMath.subtract(a, b)).toBe(BigInt(8_400_000));

        // Multiplication (ex: 10.50€ * 2.00)
        expect(SovereignMath.multiply(a, SovereignMath.toMicrounits(2))).toBe(BigInt(21_000_000));
        
        // Division (ex: 10€ / 3.00)
        const ten = SovereignMath.toMicrounits(10);
        const divided = SovereignMath.divide(ten, SovereignMath.toMicrounits(3));
        expect(divided).toBe(BigInt(3_333_333)); // 3.333333€
    });

    it('SC-5: Sécurité de Division par Zéro', () => {
        expect(() => SovereignMath.divide(BigInt(100) as any, BigInt(0) as any)).toThrow('FISCAL_DIVISION_BY_ZERO');
    });
});
