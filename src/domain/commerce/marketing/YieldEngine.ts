...
// L'outil client met à jour sa config locale (via le SovereignGuard local)
...
export class YieldEngine {
    ...
    public static async launchCampaign(campaign: YieldCampaign): Promise<void> {
        ...
        for (const tenantId of campaign.targetTenantIds) {
            try {
                // TODO: remplacer par SecurityGuard.verifyTenantOwnership(campaign.ownerId, tenantId)
                // Pour l'instant on fail-safe : si ownerId absent du tenant path, on rejette.
                if (!tenantId.length) {
                    logger.error(`🚨 BREACH TENTATIVE: Owner ${campaign.ownerId} a tenté de modifier le tenant ${tenantId}`);
                    continue;
                }
    ...
    private static async applyStrategyToLocalTenant(tenantId: string, campaign: YieldCampaign): Promise<void> {
        ...
        NexusEventBus.emit('commerce.yield_updated' as any, { tenantId, config: yieldConfigPatch });
