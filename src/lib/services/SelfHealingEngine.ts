...
export const SelfHealingEngine = {
  ...
  async auditAndHeal<T>(atom: import('jotai').PrimitiveAtom<T>, expectedHash: string, persistencePath?: string) {
    ...
    const currentHash = this.calculateCRC(currentState as unknown as import('@nexus/contracts/nexus-contract').SovereignData);
   ...
   */
  calculateCRC(data: import('@nexus/contracts/nexus-contract').SovereignData | import('@nexus/contracts/nexus-contract').SovereignValue): string {
