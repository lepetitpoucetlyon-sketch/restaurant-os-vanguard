...
export type AuditModule = 'kitchen' | 'accounting' | 'inventory' | 'staff' | 'haccp' | 'system' | 'orchestration' | 'fleet' | 'ops' | 'finance' | 'human' | 'compliance' | 'admin' | 'crm' | 'security' | 'fiscal' | 'commerce' | 'facility' | 'logistics' | 'intelligence' | 'legal';
...
class EmpireAuditLogger {
    ...
    public log(event: AuditEvent) {
        setTimeout(() => {
            ...
            try {
                ...
                try {
                    logger.info(event.action, payload as import("@/shared/nexus/contracts").SovereignMap);
                } catch (_e) {
     ...
     */
    public error(error: Error | string, module: AuditModule, context?: import("@/shared/nexus/contracts").SovereignData) {
        this.log({
            module,
            ...
            details: {
                ...
                ...(context as import("@/shared/nexus/contracts").SovereignData)
            },
            timestamp: new Date()
        });
    }
