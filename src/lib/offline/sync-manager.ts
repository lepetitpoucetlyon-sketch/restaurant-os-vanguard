...
async function executeMutationOp(op: SyncOperation): Promise<void> {
    ...
    if (op.action === 'CREATE') {
        ...
        await Nexus.adapter.update(fullPath, op.payload as Partial<import('@nexus/contracts/nexus-contract').SovereignData>);
    }
}
...
async function executeFallbackOp(op: SyncOperation): Promise<void> {
    ...
    if (op.action === 'UPDATE') await Nexus.adapter.update(fullPath, op.payload as Partial<import('@nexus/contracts/nexus-contract').SovereignData>);
}
...
export class SyncManager {
    ...
    private static async executeOperation(op: SyncOperation) {
        if (op.type === 'NF525_PAYMENT') {
            const payload = op.payload as { instructions: Array<{ method: string; path: string; data: import('@nexus/contracts/nexus-contract').SovereignData }> };
            ...
            const payload = op.payload as import('@nexus/contracts/nexus-contract').SovereignData;
