...
export const RuntimeValidator = {
   ...
   */
  validate<T>(data: import("@/shared/nexus/contracts").SovereignValue, schema: 'Cents' | 'Quantity' | 'Rate'): T | null {
   ...
   */
  validateOrder(orderData: import("@/shared/nexus/contracts").SovereignData) {
    ...
    return {
      ...
      ...(orderData.totalInCents !== undefined
        ? { totalInCents: RuntimeValidator.validate<Cents>(orderData.totalInCents as import("@/shared/nexus/contracts").SovereignValue, 'Cents') }
        : {}),
      items: items.map((item: import("@/shared/nexus/contracts").SovereignData) => ({

        ...item,
        priceInCents: RuntimeValidator.validate<Cents>(item.priceInCents as import("@/shared/nexus/contracts").SovereignValue, 'Cents'),
        quantity: RuntimeValidator.validate<Quantity>(item.quantity as import("@/shared/nexus/contracts").SovereignValue, 'Quantity')
      }))
    };
  }
};
