...
function applyTimeSyncData(ts: { offset: number; isSynced: boolean }, data: import('@nexus/contracts/nexus-contract').SovereignData): void {
...
export const TimeSync = {
  ...
  async init() {
    ...
    this.private_unsub = Nexus.adapter.onSnapshot<import('@nexus/contracts/nexus-contract').SovereignData>(syncPath, (data) => {
