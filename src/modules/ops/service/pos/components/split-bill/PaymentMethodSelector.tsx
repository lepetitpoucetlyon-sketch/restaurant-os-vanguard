    ...
    return (
        <div className="flex-1 p-16 flex flex-col items-center justify-center space-y-16 animate-in fade-in slide-in-from-bottom-8 duration-700 relative z-10">
            <div className="text-center space-y-8">
                ...
                <h2 className="text-3xl font-serif font-black text-text-primary italic tracking-tighter">
                    {t('pos.split.convive_signature_title')}<br />{t('pos.split.convive')} {payingConvive + 1}
                </h2>
                <div className="relative">
                    <p className="text-7xl font-serif font-black text-accent-gold italic drop-shadow-glow">
                        {formatCurrency(SovereignMath.toCents(BigInt(conviveTotal)))}
                    </p>
                    ...
                    <div className="w-full p-6 rounded-3xl border border-accent-gold/20 bg-accent-gold/5 flex flex-col items-center gap-4 animate-in zoom-in duration-500">
                        {terminalState === 'pending' || terminalState === 'manual_wait' ? (
                            ...
                            <>
                                ...
                                <button onClick={onResetTerminalState} className="px-6 py-2 rounded-full bg-surface-card border border-white/10 text-text-primary/50 text-[10px] font-bold uppercase hover:bg-white/10 transition-colors mt-2">Réessayer</button>
                            </>
                        ) : null}
                    </div>
                )}
