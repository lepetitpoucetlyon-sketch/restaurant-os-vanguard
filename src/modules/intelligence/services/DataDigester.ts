...
export class DataDigester {
  ...
  static async digestOrder(rawData: SovereignData, options: { isLegacy?: boolean } = {}): Promise<ExternalOrder | null> {
    try {
      ...
      logger.info('[DataDigester] Starting ingestion...', { source: (sanitized as import("@/shared/nexus/contracts").SovereignData).source as string, isLegacy: !!options.isLegacy });
      
      if (options.isLegacy) {
        (sanitized as import("@/shared/nexus/contracts").SovereignData)._metadata = { 
            isLegacy: true, 
