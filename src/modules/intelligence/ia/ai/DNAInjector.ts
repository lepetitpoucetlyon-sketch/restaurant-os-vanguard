...
export class DNAInjector {
    ...
    static async getTenantDNA(tenantId: string): Promise<string> {
        ...
        try {
            ...
            try {
                const results = await Nexus.adapter.query<import('@nexus/contracts/nexus-contract').SovereignData>('tenant_knowledge', {
            ...
            const staticRules = (staticConfig as import('@nexus/contracts/nexus-contract').TenantConfig)?.metadata?.description || staticConfig?.name || "";
