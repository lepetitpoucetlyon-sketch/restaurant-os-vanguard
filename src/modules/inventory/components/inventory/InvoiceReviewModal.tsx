...
export function InvoiceReviewModal({ data, onClose, onSaveComplete }: InvoiceReviewModalProps) {
    ...
    const [matches, setMatches] = useState<VisionMatchResult[]>(() => 
        data.items.map(item => InventoryVisionService.findBestMatch(item, ingredients as any))
    );
    ...
    return (
        <div className="fixed inset-0 z-[100] bg-black/90 backdrop-blur-xl flex items-center justify-center p-6">
            <motion.div 
                ...
                className="w-full max-w-5xl bg-[#0B0B0C] border border-white/10 rounded-[3rem] flex flex-col max-h-[90vh] overflow-hidden shadow-[0_30px_100px_rgba(0,0,0,0.8)]"
            >
                {/* Header */}
                <div className="p-10 border-b border-white/5 flex items-center justify-between bg-white/[0.02]">
                    <div className="flex items-center gap-6">
                        <div className="w-16 h-16 rounded-2xl bg-amber-500 text-black flex items-center justify-center shadow-2xl shadow-amber-500/20">
                            <ShoppingCart size={32} />
                        </div>
                        <div>
                            <div className="flex items-center gap-3 mb-1">
                                <span className="text-[10px] font-black uppercase tracking-[0.4em] text-neutral-500">Multimodal Audit Results</span>
                                <div className="px-3 py-1 bg-emerald-500/10 border border-emerald-500/20 rounded-full">
                                    <span className="text-[8px] font-bold text-emerald-500 uppercase tracking-widest">IA Verified</span>
                                </div>
                            </div>
                            <h2 className="text-4xl font-serif italic text-white uppercase tracking-tighter">Revue de Facture : {data.supplierName}</h2>
                        </div>
                    </div>
                    <button onClick={onClose} className="p-4 hover:bg-white/5 rounded-full transition-colors text-neutral-500 hover:text-white">
                        <X size={24} />
                    </button>
                </div>
                ...
                <div className="flex-1 overflow-y-auto p-10 custom-scrollbar">
                    <table className="w-full border-separate border-spacing-y-4">
                        <thead>
                            <tr className="text-[10px] font-black text-neutral-600 uppercase tracking-[0.3em]">
                        ...
                        <tbody>
                            {matches.map((match, i) => (
                                <motion.tr 
                                    ...
                                    className="group bg-white/[0.03] hover:bg-white/[0.05] transition-all rounded-2xl overflow-hidden"
                                >
                ...
                <div className="p-10 border-t border-white/5 flex items-center justify-between bg-white/[0.02]">
                    <div className="space-y-1">
                        <p className="text-[9px] font-bold text-neutral-500 uppercase tracking-widest">Total Facture (Estimé)</p>
                    ...
                    <div className="flex gap-4">
                        <Button onClick={onClose} variant="ghost" className="px-8 py-6 text-[10px] uppercase font-black tracking-widest text-neutral-500 hover:text-white">Annuler</Button>
                        <Button 
                            onClick={handleSave}
                            className="px-10 py-6 bg-white text-black text-[10px] uppercase font-black tracking-[0.2em] rounded-2xl flex items-center gap-3 hover:scale-105 active:scale-95 transition-all shadow-[0_20px_50px_rgba(255,255,255,0.1)]"
                        >
                            <Save size={18} />
