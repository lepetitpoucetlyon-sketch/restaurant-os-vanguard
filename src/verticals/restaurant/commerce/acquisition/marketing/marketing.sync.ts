...
export const MarketingSyncService = {
  ...
  init(tenantId: string, store: JotaiStore) {
    ...
    this.private_listeners.marketing = Nexus.adapter.onSnapshot(
        ...
        (data: Campaign[]) => {
          if (store) store.set(marketingCampaignsNodeAtom, (prev) => updateNexusNode(prev, { data: (data as unknown as import('@/verticals/restaurant/commerce/acquisition/marketing/types').MarketingCampaign[]) || [], loading: false }));
        },
        {
    ...
    this.private_listeners.social = Nexus.adapter.onSnapshot(
        path('socialAccounts'),
        (data: SocialAccount[]) => {
          if (store) store.set(socialAccountsNodeAtom, (prev) => updateNexusNode(prev, { data: (data as unknown as import('@/verticals/restaurant/commerce/acquisition/marketing/types').SocialAccount[]) || [], loading: false }));
        },
        {
    ...
    this.private_listeners.quotes = Nexus.adapter.onSnapshot(
      path('quotes'),
      (data: Quote[]) => {
        if (store) store.set(quotesNodeAtom, (prev) => updateNexusNode(prev, { data: (data as unknown as import('@/verticals/restaurant/commerce/acquisition/marketing/quotes.types').Quote[]) || [], loading: false }));
      },
      {
