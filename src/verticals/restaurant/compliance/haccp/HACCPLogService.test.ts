...
// On utilise vi.spyOn sur l'adapter existant (MockAdapter) pour éviter les conflits avec setup.ts
const mockSet = vi.spyOn(Nexus.adapter, 'set').mockResolvedValue(undefined);
const mockGenerateId = vi.spyOn(Nexus.adapter, 'generateId').mockImplementation((path: string) => `${path.split('/').pop()}-id`);
