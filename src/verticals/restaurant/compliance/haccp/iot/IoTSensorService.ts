...
export const IoTSensorService = {
  ...
  async connectBLESensor(sensor: IoTSensor): Promise<SensorReading | null> {
    ...
    const tempChar = await service.getCharacteristic(BLE_TEMPERATURE_CHAR) as unknown as { readValue: () => Promise<DataView> };
    ...
    try {
      const humChar = await service.getCharacteristic(BLE_HUMIDITY_CHAR) as unknown as { readValue: () => Promise<DataView> };
