...
import type { StockItem } from "@/verticals/restaurant/logistics/inventory/inventory/types";
...
function ModalShell({ onClose, children }: { onClose: () => void; children: React.ReactNode }) {
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={onClose}>
            <div className="bg-surface-base rounded-xl border border-border shadow-xl p-6 w-80" onClick={(e) => e.stopPropagation()}>
                {children}
            </div>
        </div>
    );
