...
import { ConvivePayment } from "./SplitBillTypes";

interface SplitBillConviveListProps {
    convivePayments: ConvivePayment[];
    getConviveTotal: (conviveIndex: number) => number;
    handlePayConvive: (conviveIndex: number) => void;
}

export function SplitBillConviveList({
    convivePayments,
    getConviveTotal,
    handlePayConvive
}: SplitBillConviveListProps) {
    const { t } = useLanguage();
