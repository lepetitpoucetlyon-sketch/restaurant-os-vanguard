 ...
 */
export function useNexusMutation<T extends { id: string } & import("@nexus/contracts/nexus-contract").SovereignMap>(
    ...
    const mutate = useCallback(async (
        ...
        if (!options.skipValidation && (action === 'SET' || action === 'UPDATE')) {
            const validation = validateMutation(moduleId, key, (payload || {}) as import("@nexus/contracts/nexus-contract").SovereignData);
