...
export async function handleMccHealthPing(payload: Record<string, unknown>) {
  const { tenantId, status, ...metrics } = payload as { tenantId: string; status: string } & Record<string, unknown>;
  const now = new Date();
...
export function registerMccHealthPingHandler() {
  return NexusEventBus.on(
    'mcc.health_ping',
    handleMccHealthPing as Parameters<typeof NexusEventBus.on>[1],
