...
export class BudgetTrackingService {
    ...
    static async generateMonthlyBudgetReport(tenantId: string, yearMonth: string): Promise<BudgetReport> {
        ...
        try {
            ...
            const entries = await Nexus.adapter.get<Record<string, { date: string, type: string, amount: number, lines?: { side: string, accountId: string, analyticalAxis?: string, amountInCents: number }[] }>>(
