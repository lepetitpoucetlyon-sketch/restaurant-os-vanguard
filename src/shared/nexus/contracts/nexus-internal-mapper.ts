...
export interface Table extends SovereignNode {
  number: string;
  seats: number;
  status: 'free' | 'seated' | 'ordered' | 'eating' | 'paying' | 'dirty' | 'reserved' | 'cleaning' | 'locked' | string;
  shape: 'rect' | 'circle' | 'oval' | 'hexagon';
  x: number;
  y: number;
  width?: number;
  height?: number;
...
export interface Order extends SovereignNode {
  status: 'draft' | 'new' | 'ordered' | 'preparing' | 'ready' | 'delivered' | 'cancelled' | 'paid' | string;
  tableId?: string;
  tableNumber: string;
  serverName?: string;
  timestamp: string;
...
export type OrderStatus = 'draft' | 'new' | 'ordered' | 'preparing' | 'ready' | 'delivered' | 'cancelled' | 'paid' | string;
export type ModuleId = string;

export interface Ingredient extends SovereignNode {
  name: string;
  unit: string;
  cost: number;
  category: string;
...
export function isIngredient(node: SovereignNode): node is Ingredient {
  return (
    ...
    typeof node.shelfLifeDays === 'number'
  );
}
...
export function toLegalInvoice(node: SovereignNode): LegalInvoice {
  if (!isLegalInvoice(node)) {
    throw new Error(`Cannot convert SovereignNode to LegalInvoice: missing required properties`);
  }
  return node;
}
...
export class NexusInternalMapper {
  static mapToTable(node: SovereignNode): Table { return toTable(node); }
  static mapToOrder(node: SovereignNode): Order { return toOrder(node); }
  static mapToProduct(node: SovereignNode): Product { return toProduct(node); }
  static mapToRecipe(node: SovereignNode): Recipe { return toRecipe(node); }
  static mapToIngredient(node: SovereignNode): Ingredient { return toIngredient(node); }
  static mapToReservation(node: SovereignNode): Reservation { return toReservation(node); }
  static mapToQuote(node: SovereignNode): Quote { return toQuote(node); }
  static mapToCampaign(node: SovereignNode): Campaign { return toCampaign(node); }
  static mapToFloor(node: SovereignNode): Floor { return toFloor(node); }
  static mapToZone(node: SovereignNode): Zone { return toZone(node); }
  static mapToLegalInvoice(node: SovereignNode): LegalInvoice { return toLegalInvoice(node); }
  static mapToCustomer(node: SovereignNode): Customer { return toCustomer(node); }
