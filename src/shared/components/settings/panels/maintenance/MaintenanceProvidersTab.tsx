  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {providers.map((prov) => (
          <div key={prov.id} className="p-5 rounded-2xl bg-surface-card border border-border-default space-y-3">
            <div className="flex items-start justify-between">
              <div className="space-y-0.5">
                <h4 className="text-sm font-bold text-text-primary">{prov.name}</h4>
                <p className="text-xs text-emerald-400">{prov.specialty}</p>
              </div>
              <span className="px-2 py-0.5 rounded-md bg-surface-glass text-nano font-mono text-text-muted">
                {prov.contractNumber || 'Contrat Standard'}
              </span>
            </div>

            <div className="space-y-1.5 text-xs text-text-secondary pt-2 border-t border-border-default">
              <div className="flex items-center gap-2">
                <Phone className="w-3.5 h-3.5 text-text-muted" />
                <span className="font-semibold text-text-primary">{prov.phone}</span>
              </div>
              {prov.email && (
                <div className="flex items-center gap-2">
            ...
            <div className="pt-2 flex items-center gap-1.5 flex-wrap">
              ...
              {prov.assignedZones.map((z: RestaurantZone) => (
                <span key={z} className="px-2 py-0.5 rounded bg-surface-glass border border-border-default text-nano text-text-secondary">
