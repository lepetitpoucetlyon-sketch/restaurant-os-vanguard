    ...
    return (
        <div className="rounded-2xl border border-accent-gold/20 bg-accent-gold/5 p-6 space-y-5">
            <div className="flex items-center justify-between">
                <p className="text-micro font-black uppercase tracking-widest text-accent-gold">
                    {step === "adapter" ? "1 · Type de terminal" : step === "connection" ? "2 · Connexion" : "3 · Configuration"}
                </p>
                <button
                    onClick={onCancel}
                    className="text-nano text-text-muted hover:text-text-primary uppercase tracking-wider font-black"
                >
                    Annuler
                </button>
            </div>
            ...
            {step === "adapter" && (
                <div className="grid grid-cols-2 gap-2">
                    {(Object.keys(ADAPTER_LABELS) as TerminalAdapterType[]).map(a => (
                        ...
                        >
                            <p className="text-micro font-black text-text-primary">{ADAPTER_LABELS[a]}</p>
                            {a === "manual" && <p className="text-nano text-text-muted">Fallback — pas de TPE physique</p>}
                            {a === "simulator" && <p className="text-nano text-text-muted">Tests en développement</p>}
                            {a === "stripe" && <p className="text-nano text-text-muted">M2, WisePOS E — BLE / LAN</p>}
                            {a === "sumup" && <p className="text-nano text-text-muted">Air (BLE) ou Solo (3G)</p>}
                            {a === "worldline" && <p className="text-nano text-text-muted">Ingenico — banques françaises</p>}
                            {a === "adyen" && <p className="text-nano text-text-muted">Cloud — bientôt disponible</p>}
                        </button>
                    ))}
                </div>
            )}
            ...
            {step === "connection" && (
                <div className="space-y-3">
                    ...
                    <button onClick={() => setStep("adapter")} className="text-nano text-text-muted hover:text-text-primary uppercase tracking-wider font-black">
                        ← Retour
                    </button>
                </div>
            )}
                ...
                <div className="space-y-4">
                    ...
                    <label className="flex items-center gap-2 cursor-pointer">
                        <input
                            ...
                            className="rounded"
                        />
                        <span className="text-micro font-black uppercase tracking-wider text-text-muted">Définir comme terminal par défaut</span>
                    </label>
                    <div className="flex gap-3 pt-2">
                        <button onClick={() => setStep("connection")} className="px-4 h-10 rounded-xl border border-border text-micro font-black uppercase tracking-wider text-text-muted hover:text-text-primary transition-colors">
                            ← Retour
                        </button>
                        <button
                            onClick={onSave}
                            disabled={!form.name.trim()}
                            className="flex-1 h-10 rounded-xl bg-accent-gold text-text-primary text-micro font-black uppercase tracking-widest hover:bg-accent-gold/90 transition-colors disabled:opacity-40"
                        >
