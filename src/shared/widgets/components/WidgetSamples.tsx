    ...
    return (
        <div className={`p-4 rounded-2xl bg-surface-card border border-border-default flex flex-col justify-between h-full ${className ?? ''}`}>
            <div className="flex items-center justify-between">
                <span className="text-[11px] font-semibold uppercase tracking-wider text-text-secondary">CA du Jour</span>
            ...
            <div className="my-2">
                ...
                <div className="text-[11px] text-emerald-500 flex items-center gap-0.5 mt-0.5 font-medium">
...
export function WeatherWidget({ className }: WidgetProps) {
    return (
        <div className={`p-4 rounded-2xl bg-surface-card border border-border-default flex flex-col justify-between h-full ${className ?? ''}`}>
            <div className="flex items-center justify-between">
                <span className="text-[11px] font-semibold uppercase tracking-wider text-text-secondary">Terrasse & Météo</span>
                <span className="text-base">☀️</span>
            </div>
            <div className="my-2">
                <div className="text-2xl font-black text-text-primary">24°C</div>
                <div className="text-[11px] text-text-secondary mt-0.5 font-medium">Ensoleillé • Vent faible (8 km/h)</div>
            </div>
            <div className="text-[10px] text-emerald-500 border-t border-border-subtle pt-2 font-medium">
...
export function ProbeWidget({ className }: WidgetProps) {
    return (
        <div className={`p-4 rounded-2xl bg-surface-card border border-border-default flex flex-col justify-between h-full ${className ?? ''}`}>
            <div className="flex items-center justify-between">
                <span className="text-[11px] font-semibold uppercase tracking-wider text-text-secondary">Sondes Frigo HACCP</span>
                <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
            </div>
            <div className="grid grid-cols-2 gap-2 my-2">
                <div className="p-2 rounded-xl bg-surface-bg border border-border-subtle">
                    <div className="text-[10px] text-text-muted">Chambre Froide Pos.</div>
                    <div className="text-sm font-bold font-mono text-emerald-500 mt-0.5">+3.2°C</div>
                </div>
                <div className="p-2 rounded-xl bg-surface-bg border border-border-subtle">
                    <div className="text-[10px] text-text-muted">Congélateur Neg.</div>
...
export function ReviewsWidget({ className }: WidgetProps) {
    return (
        <div className={`p-4 rounded-2xl bg-surface-card border border-border-default flex flex-col justify-between h-full ${className ?? ''}`}>
            <div className="flex items-center justify-between">
                <span className="text-[11px] font-semibold uppercase tracking-wider text-text-secondary">Avis Récents</span>
            ...
            <div className="text-[10px] text-text-secondary border-t border-border-subtle pt-2 flex justify-between">
