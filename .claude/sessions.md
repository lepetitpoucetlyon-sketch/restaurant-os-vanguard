...
| p4-finance | §5 Phase 5 P4 finance core — **AUDIT (lecture seule)** : 205 InCents `modules/finance` vérifiés fichier par fichier. VERDICT : P4 déjà complet — tous les InCents sont moitié-cents de dual-write, snapshot NF525 gelé, param d'entrée cents→µ, ou lecture µ-first. 0 écriture cents-only. `ProcurementBridge` L69 déjà corrigé (dual-write L67). `calculateVAT` cents @deprecated 0 appelant. Aucun code touché. Doc : `PLAN_COMPLET.md` §5.7 en-tête 🟠→🟢 + retrait P4 du chemin critique. | 2026-08-12 | terminée | 2026-08-12 | terminée |
