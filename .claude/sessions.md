...
|---|---|---|---|
| `restaurant-ui-refonte` (Claude Code & Antigravity) | Refonte UI & Raccordement 50+ chantiers dormants LIVRÉS (V0-V5.2 + 5 vagues : /suppliers SupplierHub+AutoProcurement, /kiosk KioskPage, /hygiene, /pms PmsPage, /migration FEC+Reservations, GMAO HardwareOnboarding, Z-Report EndOfDay, HACCP RecallView+ReceptionWizard+Perishables, TipPoolManager, 14 vues réelles Finance, 6 onglets MCC Forge/Study/BlindSpot/Qualif/Derivers/Scrape). `src/app/(admin)/admin/mcc/**` + `src/app/(client)/(ops)/**` + `src/modules/**`. tsc 0 erreur, verify:gates vert. | 2026-08-23 | active |
