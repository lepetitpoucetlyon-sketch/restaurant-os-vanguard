...
| rapatriement-shared-etape8 | Étape 8 finale TERMINÉE : 8c hooks intelligence → `modules/intelligence/hooks` (+ suppression proxy `useNexusFleet` FIXME + wrappers morts), 8d hooks design → `design/hooks`, 8e `useDLQQuarantine` → `orchestration/hooks` + suppression `useConnector`/`useManagement` morts, 8g barrel `@/shared/hooks` (126 importeurs) → `@/kernel/hooks`. `src/shared/` = `schemas/` seulement. Commits `680f7498f`→`1c79ede5f`. Gate: TSC=0, cycles=2 (baseline), inversions=0, barrel=0. | 2026-08-12 | terminée |
