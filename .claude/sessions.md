...
| rapatriement-shared-etape8 | Étape 8 finale : vidage `src/shared/{providers,hooks,contexts}` → `kernel/{providers,hooks,nexus/contracts/settings}` + `design/{providers,hooks}` + `modules/intelligence/…/hooks` + `orchestration/hooks` ; élimination barrel `@/shared/hooks` (126 importeurs) → `@/kernel/hooks`. `schemas/` reste. Suit `PLAN_RAPATRIEMENT_SHARED_ETAPE8.md`. | 2026-08-12 | active |
